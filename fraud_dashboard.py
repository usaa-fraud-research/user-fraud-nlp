...
def main():
   ...
   if year:
       ...
       today = date.today()
       # --- Build 53-week range (Sun-Sat) ---
       # Get Sunday of the current week (weekday 6 is Sun)
       today_weekday_offset = (today.weekday() + 1) % 7
       this_week_start_date = today - timedelta(days=today_weekday_offset)
       grid_start_date = this_week_start_date - timedelta(weeks=52)
       grid_end_date = this_week_start_date + timedelta(days=6)
       all_days = pd.date_range(start=grid_start_date, end=grid_end_date, freq="D")
       ...
       for m in range(11, 24):
           if m == 12:
               ...
               html += f"<span>{calendar.month_abbr[m%12]}</span>"
